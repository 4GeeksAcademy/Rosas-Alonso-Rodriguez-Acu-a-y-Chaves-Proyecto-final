...
const getState = ({ getStore, getActions, setStore }) => {
	return {
		...
		actions: {
			...
			addNewPet: (newPet, navigate) => {


				const token = sessionStorage.getItem('token');
				console.log(token)
				fetch(`${process.env.BACKEND_URL}/create_pet`, {
					method: 'POST',
			...
			getAllPets: async () => {
				try {
					let response = await fetch(`${process.env.BACKEND_URL}/pets`)
					let data = await response.json()
					setStore({ fetchedPets: data.data });
					console.log("Array de mascotas: ", data.data);
				} catch (error) {
					console.log("Error fetching pets:", error)
				}
			},
			getAllPetPosts: async () => {
				try {
					let response = await fetch(`${process.env.BACKEND_URL}/pet_post`)
					let data = await response.json()
					setStore({ fetchedPetPosts: data.data })
					console.log("Array de posts de mascotas: ", data.data);
				} catch (error) {
			...
			getPet: async (id) => {
				try {
					let response = await fetch(`${process.env.BACKEND_URL}/pet/${id}`)
					let data = await response.json()
					setStore({fetchedPet: data.data})
					console.log("Aqui esta la mascota solicitada: ", data.data);					
				} catch (error) {
